...
//----------------------------------------------
// ============================================== 
  var $menulink = $('[data-toggle="menu"]');
  var $wrap = $('#wrap');
  var $panelState = 'right-hidden';
  var $menuState = 'menu-visible';
  var $detailsLink = $('[data-toggle="details"]'); 
  //
 $menulink.click(function(e) {
    $wrap.removeClass('show-right').toggleClass('show-menu');
    $panelState = 'right-hidden';
    $menuState = 'menu-visible';
    // If user is clicking a menu item
        if ( !$(e.target).closest($menulink).hasClass('active') ) {
 //   if (!$(e.target).hasClass('menu-toggle') && !$(e.target).hasClass('active')) {      
       $menulink.removeClass('active');    
       $(e.target).closest($menulink).toggleClass('active');
      $('#content--list').hide(0).delay(250).show(0);
      $('#content--details').hide();
      $detailsLink.removeClass('active');
     
    }
     else if ($(e.target).hasClass('menu-toggle')) {
      $detailsLink.removeClass('active');
    } 
    else {
      return false;
...
//
  $detailsLink.click(function(e) {
    // If Right panel is not visible
    if ($panelState !== 'right-shown'){
      $detailsLink.toggleClass('btn-active');
      $wrap.removeClass('show-menu').toggleClass('show-right');
      $panelState = 'right-shown';
      $('#content--details').show();
    }
    // If user clicks on an item that's not active
    if ( !$(e.target).closest($detailsLink).hasClass('active') ) {
      $detailsLink.removeClass('active');
      $(e.target).closest($detailsLink).toggleClass('active');
      $('#content--details').hide();
      setTimeout(function() {
        $('#content--details').show();
      }, 500);
    }
    if ( $(e.target).hasClass('details-toggle')) {
      $wrap.toggleClass('show-right');
      $panelState = 'right-hidden';
    }
    // If user clicks on an item that IS active
    else {
      return false;
    }
  });
